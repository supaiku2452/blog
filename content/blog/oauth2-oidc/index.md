...
従来のクラサバ型の認証モデルでは、クライアントからサードパーティアプリケーションにアクセスする際は、クライアントにサードパーティアプリケーションのクレデンシャル情報を入力してもらい、それを利用してリソースにアクセスしていました。

しかし、この認証モデルではセキュリティのリスク低下のサーバーがパスワード形式の認証方法をサポートしなければならないなどの問題があります。
...
OAuth2.0 はこのような状況の時に、第 3 者へユーザー ID・パスワードを渡さずに、制限されたリソースにアクセスできる仕組みを提供します。ただし、OAuth2.0 では認証に関する部分は一切しようとして定められていないので、認可のフレームワークであることに注意してください。

認証のフレームワークではないと記載しているのは、[RFC 6749 3.1. Authorization Endpoint](https://tools.ietf.org/html/rfc6749#section-3.1)に、以下の一文があるからです。
